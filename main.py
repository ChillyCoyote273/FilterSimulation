...
def filter_data(us: np.ndarray[float], ys: np.ndarray[float],
                k_v: float = 1, k_a: float = 1, k_g: float = 0.1,
                q: np.ndarray[float] = np.array([[1, 0], [0, 1]]),
                r: float = 1, dt: float = 0.05) -> np.ndarray[float]:
...
def main() -> None:
    t, pow, pos, vels = generate_data(600, k_v = 0.01, k_a = 0.001, seed = 0)
